...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .provider-evidence-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:9px;margin-top:12px}.provider-evidence{padding:10px 11px;border:1px solid #e8e1f2;border-radius:13px;background:#fcfbfe}.provider-evidence.final{border-color:#cdb8f2;background:#faf7ff}.provider-evidence-head{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:7px;color:#6e518e;font-size:12px}.provider-evidence-head strong{color:var(--ink);font-size:12px}.provider-facts{display:flex;flex-wrap:wrap;gap:6px}.provider-facts .fact{font-size:10px}
...
<body>
  ...
  <script>
    ...
    function providerEvidence(label,foundKplus,foundSettlement,amounts,extra='',final=false){const amountText=amounts===null?'ไม่มีข้อมูล':((amounts||[]).join(', ')||'ไม่พบ');return '<section class="provider-evidence'+(final?' final':'')+'"><div class="provider-evidence-head"><strong>'+label+'</strong><span>'+extra+'</span></div><div class="provider-facts">'+fact('KPLUS/K+',foundKplus)+fact('SETTLEMENT',foundSettlement)+'<span class="fact">ยอด: '+escapeHtml(amountText)+'</span></div></section>'}
    ...
    function renderLogs(items){
      ...
      logList.innerHTML=items.map(item=>{
        ...
        const providerParts=String(item.provider||'').split('+');
        const providerCards=[];
        if(providerParts.includes('paddleocr')) providerCards.push(providerEvidence('PaddleOCR',providerParts.includes('ocrspace')?null:item.foundKplus,providerParts.includes('ocrspace')?null:item.foundSettlement,providerParts.includes('ocrspace')?null:item.detectedAmounts));
        if(providerParts.includes('ocrspace')) providerCards.push(providerEvidence('OCR.space',item.ocrspaceFoundKplus,item.ocrspaceFoundSettlement,item.ocrspaceDetectedAmounts,ocrKey.replace(/<[^>]+>/g,'')));
        if(item.aiProvider) providerCards.push(providerEvidence('Workers AI Vision',item.aiFoundKplus,item.aiFoundSettlement,item.aiDetectedAmounts,item.aiConfident?'มั่นใจ':'ไม่มั่นใจ'));
        const providerEvidenceHtml=providerCards.length?'<div class="provider-evidence-grid">'+providerCards.join('')+providerEvidence('ผลรวมสุดท้าย',item.foundKplus,item.foundSettlement,item.detectedAmounts,'',true)+'</div>':'';
        return '<article class="log-row"><div class="log-main"><div class="log-cell"><small>เวลา</small><strong>'+escapeHtml(formatTime(item.updatedAt))+'</strong></div><div class="log-cell"><small>เลขงาน</small><strong>'+escapeHtml(item.jobNumber)+'</strong></div><div class="log-cell"><small>ผล</small><strong class="status-pill '+status[1]+'">'+escapeHtml(status[0])+'</strong></div><div class="log-cell reason"><small>เหตุผล</small><strong>'+escapeHtml(item.reason||'กำลังรอประมวลผล')+'</strong></div></div><div class="log-facts"><span class="fact">ตรวจด้วย: '+escapeHtml(providerName(item.provider))+'</span>'+ocrKey+replyTokenAge+imageSet+'<span class="fact">ผลสุดท้าย ยอด: '+escapeHtml(amounts)+'</span></div>'+providerEvidenceHtml+evidenceDetail+ocrDetail+aiDetail+'</article>'
      }).join('')
    }
