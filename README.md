...
1. รับเลขงาน 8 หลัก แยกตาม `region + LINE user ID` และเก็บเลขงานล่าสุดไว้ 30 นาที.
...
7. มีหลักฐานครบแต่ยอดไม่ตรง => `needs_fallback` (บันทึก Log แต่ยังไม่เรียก provider อื่น).
8. ระหว่างรับเลขงานและรับรูป ระบบไม่ส่งข้อความตอบรับ.
9. ระบบส่ง LINE Push Flex Message เฉพาะเมื่อมีรูปใดรูปหนึ่งในงานตรวจผ่าน และใช้ D1 claim เพื่อไม่ตอบซ้ำแม้หลายรูปผ่านพร้อมกัน.
10. รูปที่เริ่มประมวลผลหลังงานแจ้งผ่านแล้วจะหยุดตรวจเพื่อประหยัดโควตา แต่ยังมีรายการใน Log.
